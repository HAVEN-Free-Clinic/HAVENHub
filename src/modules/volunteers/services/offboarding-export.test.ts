...
describe("buildOffboardingCsv", () => {
  ...
  it("exports a selection with blank departments and a VOLUNTEER role when there is no active term", async () => {
    const withNetId = await prisma.person.create({ data: { name: "NoTerm Alpha", netId: "nta01" } });
    const withContact = await prisma.person.create({
      data: { name: "NoTerm Beta", contactEmail: "b@example.com" },
    });
    ...
    expect(result.rowCount).toBe(2);
    expect(result.filename).toBe("haven-offboarding-no-term-2026-08-07.csv");
    expect(result.csv).toContain("NoTerm Alpha,nta01@yale.edu,nta01,,,VOLUNTEER");
    expect(result.csv).toContain("NoTerm Beta,b@example.com,,b@example.com,,VOLUNTEER");
  });
});
