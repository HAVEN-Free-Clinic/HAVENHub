...
export default async function QuizBuilderPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  return (
    <div className="max-w-3xl space-y-6">
        ...
        <section key={section.id} className="rounded-2xl border border-border bg-surface p-5 shadow-sm">
          ...
          <ul className="mt-4 space-y-4">
            {section.fields.map((f) => {
              ...
              return (
                <li key={f.id} className="border-t border-border-subtle pt-4 first:border-t-0 first:pt-0">
                  ...
                  >
                    {opts.map((o) => (
                      <label key={o.value} className="flex items-center gap-2 text-sm text-foreground-soft">
                        <input
                          type="radio"
                          name="correctValue"
                          value={o.value}
            ...
            {section.fields.length === 0 && <li className="text-sm text-subtle-foreground">No questions yet.</li>}
          </ul>
          <form
            action={addQuizQuestionAction.bind(null, id, section.id)}
            className="mt-5 space-y-3 border-t border-border-subtle pt-5"
          >
      ...
      <form action={addQuizSectionAction.bind(null, id)} className="flex flex-wrap items-end gap-3 rounded-2xl border border-dashed border-border-strong bg-muted/60 p-5">
