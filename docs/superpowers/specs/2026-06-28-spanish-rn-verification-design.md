...
2. **Existing imported data is grandfathered as verified-but-provisional.** Rows with the old `spanishSpeaking = true` become `verified = true` (no scheduling-coverage cliff) and also land in the interpreting-department review queue for later confirmation.
...
| Self-reported, awaiting assessment | true | false | null | yes | no |
| Grandfathered (provisional) | true | true | null | yes | yes |
...
- `updatePersonFields`: verified `false -> true` stamps verifier and timestamp; `true -> false` clears both; a grandfathered row (`verified = true, verifiedAt = null`) re-saved unchanged keeps `verifiedAt = null` (stays in queue).

Queue predicate: unit-tested across all four non-trivial states (self-reported / grandfathered / assessed-yes / assessed-no).
...
Migration verification: a person who had `spanishSpeaking = true` lands as `spanishSelfReported = true, spanishVerified = true, spanishVerifiedAt = null` (grandfathered and in queue).
...
- Grandfathered rows appear here for confirmation; recording any result removes them from the queue.
