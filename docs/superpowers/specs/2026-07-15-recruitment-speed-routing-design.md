...
- `reopenDecision(applicationId, actorId)`: inverse of a reject. Sets `decision = PENDING`, clears `decidedBy*`, `decidedAt`, `decisionNotes`. Guards: `review_all`; refuse when `decisionsReleasedAt` is set on the cycle or an emailed acceptance exists (nothing to safely reopen once the applicant was told). Audits `recruitment.application_reopen`.
