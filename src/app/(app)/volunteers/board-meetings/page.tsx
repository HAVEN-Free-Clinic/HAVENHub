...
export default async function BoardMeetingsPage({ searchParams }: PageProps) {
  ...
  const [meetings, absences] = await Promise.all([
  ...
  async function createAction(formData: FormData) {
    ...
    if (!activeTerm) redirect("/volunteers/board-meetings");
    try {
      ...
      if (err instanceof BoardAttendanceValidationError || err instanceof BoardAttendanceForbiddenError) {
        redirect(`/volunteers/board-meetings?error=${encodeURIComponent(err.message)}`);
      }
      throw err;
    }
    redirect("/volunteers/board-meetings");
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="space-y-3">
        ...
        {meetings.length === 0 ? (
          <Card pad={false} className="px-6 py-10 text-center text-sm text-muted-foreground">
            No board meetings recorded for this term yet.
          </Card>
        ) : (
          <Table>
            <THead>
            ...
            <tbody>
              {meetings.map((m) => (
                <TR key={m.id}>
                  ...
                  <TD>
                    <Link href={`/volunteers/board-meetings/${m.id}`} className="text-brand-fg hover:underline text-sm">
                      Take attendance
                    </Link>
                  </TD>
                </TR>
