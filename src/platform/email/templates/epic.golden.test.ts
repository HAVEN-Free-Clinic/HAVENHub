 ...
 * same body content as the pre-refactor epicOnboardingEmail / epicActivationEmail /
 * epicPasswordResetEmail functions. The body is injected verbatim into the branded
 * layout shell, so we assert the body content is contained in the rendered email.
 *
 * One deliberate divergence from that pre-refactor output: the greeting reads
 * "Hello Alice," rather than "Hello Alice Smith,". These templates now greet by
 * {{ personFirstName }}, like every other salutation in the app, so somebody who
 * goes by a name other than their legal one is addressed by it. The full name is
...
describe("epic templates via renderEmail (body inside branded layout)", () => {
  // ---------------------------------------------------------------------------
  // epic-onboarding NEW
  // ---------------------------------------------------------------------------
