 ...
 *
 * The exclusion is services, not `src/modules`. A thrown Error's default
 * message is not UI copy: src/modules/incidents/services/report.ts keeps its
 * own `IncidentForbiddenError` default, which no user ever sees (every catch
 * site redirects with the `forbidden` CODE, never `err.message`), and binding
 * it to a toast string would mean a copy edit silently rewriting a domain
 * exception. That is the whole of the argument, so it is now the whole of the
 * filter -- this previously ran over `src/app` and `src/platform/ui` only,
 * which also waved through every component under src/modules, where a fourth
 * copy of a refusal sentence is exactly as much real copy as one in src/app.
 * Tests are excluded too: flash.test.ts deliberately holds these sentences as
 * hardcoded literals, which is what makes it the value guard on the
 * constant.
 *
...
describe("shared refusal copy lives in SHARED_ERROR_TEXT", () => {
  it("has no second literal copy of a shared sentence in the copy layer", () => {
    const files = execSync("git ls-files 'src/**/*.ts' 'src/**/*.tsx'", { encoding: "utf8" })
      .split("\n")
      .filter(Boolean)
      // The declaration itself, and the services that own a domain exception's
      // default message. Everything else under src/ is copy and is checked.
      .filter((f) => f !== "src/platform/error-text.ts")
