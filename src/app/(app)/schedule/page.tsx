            ...
            return (
              ...
              >
                ...
                {/* The attending covering THIS shift's department, not the whole
                    ...
                    and naming another team's doctor told this member nothing
                    they could act on. Both management pages are gated, so this
                    is where the person actually working the shift finds out.
                    Absent until someone is scheduled -- and when the department
                    maps to no column at all -- rather than showing an empty
                    "Attending:" row. */}
                            ...
                            <div key={group.key}>
                              ...
                              <div className="flex flex-wrap gap-2">
                                {group.dates.map((d) => {
                                  ...
                                  return (
                                    // has-[:checked] styles the pill from the live checkbox, so a toggle
                                    // gives instant feedback. Server-only styling left the pill unchanged
                                    // on click, so people re-clicked and turned the date back off.
                                    <label key={key} className="flex items-center gap-2 rounded-full border px-3 py-1.5 text-xs transition-colors whitespace-nowrap min-h-11 cursor-pointer border-border text-muted-foreground hover:border-brand/40 has-[:checked]:border-brand has-[:checked]:bg-brand/5 has-[:checked]:text-brand-fg has-[:checked]:font-semibold">
                                      <Checkbox
                                        name="dates"
                                        value={key}
